...
This portfolio contains selected university projects that demonstrate practical work across security engineering, secure development, DevSecOps and cryptography.
...
**Security:** `SIEM` `Incident Response` `Threat Detection` `Malware Analysis` `Digital Forensics` `MITRE ATT&CK` `YARA` `Network Security` `ISO 27001` `Secure Coding`
